...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    setIsLoading(true);
    // REDIRECT UPDATE: Send to /dashboard
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res?.error) {
        ...
        // REDIRECT UPDATE: Manually push to dashboard on success
    ...
    >
      <form onSubmit={handleEmailLogin} className="space-y-5">
        ...
        <Button 
          type="submit"
          disabled={isLoading}
          className="w-full h-12 bg-[#0F172A] hover:bg-[#020617] text-white rounded-xl font-semibold text-[15px] tracking-[-0.01em] shadow-[0_4px_12px_rgba(15,23,42,0.15)] transition-all active:scale-[0.98] disabled:opacity-70"
        >
