...
export default function LoginPage() {
  ...
  return (
    <AuthLayout 
      title="Welcome back" 
      subtitle="Sign in to access your saved career paths and analysis history."
    >
      <div className="space-y-5">
        ...
        <div className="space-y-2">
            ...
            <div className="flex justify-end">
              <Link href="#" className="text-[13px] font-medium text-slate-500 hover:text-slate-800 transition-colors tracking-tight">
                Forgot password?
              </Link>
            </div>
        </div>
        ...
        {/* Submit */}
        <Button className="w-full h-12 bg-[#0F172A] hover:bg-[#020617] text-white rounded-xl font-semibold text-[15px] tracking-[-0.01em] shadow-[0_4px_12px_rgba(15,23,42,0.15)] transition-all active:scale-[0.98]">
          Sign In
        </Button>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-8">
        ...
        <SocialButton>
            <Apple className="w-5 h-5 text-slate-900 fill-current" />
        </SocialButton>
      </div>
      ...
      <div className="text-center">
        <p className="text-[14px] text-slate-500 font-normal">
            ...
            <Link href="/signup" className="font-semibold text-slate-900 hover:text-black transition-colors ml-0.5">
                Sign up
            </Link>
        </p>
      </div>
...
function SocialButton({ children }: { children: React.ReactNode }) {
    return (
        <button className="h-12 flex items-center justify-center bg-white border border-slate-200 rounded-xl hover:bg-slate-50 hover:border-slate-300 transition-all active:scale-95">
            {children}
        </button>
    )
}