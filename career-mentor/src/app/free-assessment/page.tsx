...
export default function FreeAssessmentPage() {
  ...
  useEffect(() => {
    const usage = localStorage.getItem('guest_assessment_count');
    if (usage && parseInt(usage) >= 1) {
  ...
  const submitAssessment = async () => {
    ...
    try {
      ...
      localStorage.setItem('guest_assessment_count', '1');
      
      // Redirect to Free Results
      router.push(`/free-results/${data.id}`);
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-[#0F172A] flex flex-col items-center justify-center relative overflow-hidden">
        <div className="text-center z-10 p-8">
          ...
          <h2 className="text-3xl font-bold text-white mb-4 tracking-tight">Analyzing Profile (Free Mode)</h2>
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD] text-slate-900 font-sans flex flex-col">
      ...
      <LoginGateModal 
        open={showLimitModal} 
        onOpenChange={(val) => { if(!val) router.push('/home'); }} // Go home if they close it
        title="Assessment Limit Reached"
        description="You have used your 1 free guest assessment. Please sign in to take more assessments and save your results."
      />

      <header className="h-20 px-6 md:px-12 flex items-center justify-between border-b border-slate-100 bg-white/80 backdrop-blur-sm fixed top-0 w-full z-20 mt-[70px]">
      ...
      <main className="flex-1 flex flex-col items-center justify-center pt-48 pb-24 px-6">
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="flex items-center justify-between">
                                                <span className={cn( "text-lg font-medium transition-colors", isSelected ? "text-blue-900" : "text-slate-700")}>{option}</span>
                                                {isSelected && <div className="w-6 h-6 bg-blue-600 rounded-full flex items-center justify-center"><Check className="w-4 h-4 text-white" strokeWidth={3} /></div>}
                                            </div>
                                        </button>
                                    );
                                })}
                        ...
                        {currentQuestion.type === 'slider' && (
                            <div className="bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm mt-8">
                                <div className="mb-12 text-center">
                                    <span className="text-5xl font-bold text-slate-900 tracking-tighter">{answers[currentQuestion.id] || 50}</span>
      ...
      <footer className="fixed bottom-0 w-full bg-white border-t border-slate-200 p-6 z-20">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
            <Button variant="ghost" onClick={handleBack} disabled={currentIndex === 0 || isSubmitting} className="text-slate-500 hover:text-slate-900">
                <ArrowLeft className="w-4 h-4 mr-2" /> Back
            </Button>
            <Button onClick={handleNext} disabled={!isAnswered || isSubmitting} className={cn("h-12 px-8 rounded-xl font-semibold transition-all duration-300", isAnswered ? "bg-[#0F172A] hover:bg-[#1E293B] text-white shadow-lg" : "bg-slate-100 text-slate-300 cursor-not-allowed")}>
                {currentIndex === CAREER_QUESTIONS.length - 1 ? "Complete Analysis" : "Continue"} <ChevronRight className="w-4 h-4 ml-2" />
            </Button>
        </div>
      </footer>
    </div>
